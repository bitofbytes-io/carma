...
func TestMemoryInitializesLegacyReminderOdometerOnce(t *testing.T) {
	...
	types, _ := m.ListServiceTypes(ctx)
	miles := int64(5000)
	legacy, _ := m.UpsertReminder(ctx, model.Reminder{ID: uuid.New(), VehicleID: v.ID, ServiceTypeID: types[0].ID, IntervalMiles: &miles, StartingOdometerPending: true, Enabled: true, CreatedAt: now, UpdatedAt: now})
	legacy, _ = m.UpsertReminder(ctx, model.Reminder{ID: uuid.New(), VehicleID: v.ID, ServiceTypeID: types[0].ID, IntervalMiles: &miles, Enabled: false, CreatedAt: now.Add(time.Minute), UpdatedAt: now.Add(time.Minute)})
	if !legacy.StartingOdometerPending || legacy.StartingOdometer != nil {
		t.Fatalf("legacy pending state was not preserved by edit: %+v", legacy)
	}
	_, _ = m.UpsertReminder(ctx, model.Reminder{ID: uuid.New(), VehicleID: v.ID, ServiceTypeID: types[1].ID, IntervalMiles: &miles, Enabled: true, CreatedAt: now, UpdatedAt: now})
	recordMileage := int64(50000)
	_, _ = m.CreateRecord(ctx, model.Record{ID: uuid.New(), VehicleID: v.ID, ServiceTypeID: types[2].ID, OccurredOn: now, OdometerMiles: &recordMileage, CreatedAt: now}, nil)
	...
	byType := map[uuid.UUID]model.Reminder{}
	for _, row := range rows {
		byType[row.ServiceTypeID] = row
	}
	legacy = byType[types[0].ID]
	if legacy.StartingOdometer == nil || *legacy.StartingOdometer != recordMileage || legacy.StartingOdometerPending || legacy.LatestOdometer == nil || *legacy.LatestOdometer != recordMileage {
		t.Fatalf("legacy reminder was not initialized exactly once: %+v", legacy)
	}
	unknown := byType[types[1].ID]
	if unknown.StartingOdometer != nil || unknown.StartingOdometerPending || unknown.LatestOdometer == nil || *unknown.LatestOdometer != recordMileage {
		t.Fatalf("new unknown reminder was retroactively initialized: %+v", unknown)
	}
}
...
func TestMemoryReminderStartingOdometerDoesNotAliasCallers(t *testing.T) {
	m := NewMemory()
	ctx := t.Context()
	now := time.Date(2026, 7, 31, 12, 0, 0, 0, time.UTC)
	v, _ := m.CreateVehicle(ctx, model.Vehicle{ID: uuid.New(), Nickname: "Pointers", CreatedAt: now, UpdatedAt: now})
	types, _ := m.ListServiceTypes(ctx)
	miles := int64(5000)
	starting := int64(1000)
	created, err := m.UpsertReminder(ctx, model.Reminder{ID: uuid.New(), VehicleID: v.ID, ServiceTypeID: types[0].ID, IntervalMiles: &miles, StartingOdometer: &starting, Enabled: true, CreatedAt: now, UpdatedAt: now})
	if err != nil {
		t.Fatal(err)
	}
	starting = 2000
	*created.StartingOdometer = 3000
	rows, _ := m.ListReminders(ctx, &v.ID, true)
	if len(rows) != 1 || rows[0].StartingOdometer == nil || *rows[0].StartingOdometer != 1000 {
		t.Fatalf("create aliases changed stored snapshot: %+v", rows)
	}

	replacement := int64(9000)
	updated, err := m.UpsertReminder(ctx, model.Reminder{ID: uuid.New(), VehicleID: v.ID, ServiceTypeID: types[0].ID, IntervalMiles: &miles, StartingOdometer: &replacement, Enabled: false, CreatedAt: now.Add(time.Hour), UpdatedAt: now.Add(time.Hour)})
	if err != nil {
		t.Fatal(err)
	}
	replacement = 8000
	*updated.StartingOdometer = 7000
	*rows[0].StartingOdometer = 6000
	rows, _ = m.ListReminders(ctx, &v.ID, true)
	if rows[0].StartingOdometer == nil || *rows[0].StartingOdometer != 1000 || rows[0].Enabled {
		t.Fatalf("update/return aliases changed stored snapshot: %+v", rows[0])
	}
}
